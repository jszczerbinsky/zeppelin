...
static void dumpprecomp_magic(FILE *f, BitBrd magic[64], int shift[64],
    ...
    do {
      ...
      for (int i = 0; i < 64; i++) {
        if (i == sqr) {
          ...
          strocc[i][1] = ' ';
          strmov[i][0] = (magicmoves[sqr][index] & sqr2bbrd(i)) ? 'x' : ' ';
          strmov[i][1] = ' ';
        }
      }
...
void dumpprecomp() {
  ...
  dumpprecomp_magic(f, g_precomp.rookmagic, g_precomp.rookmagicshift,
                    g_precomp.rookmagicmoves, g_precomp.rookpremask);

  printsection(f, "BISHOP MAGIC");
  dumpprecomp_magic(f, g_precomp.bishopmagic, g_precomp.bishopmagicshift,
                    g_precomp.bishopmagicmoves, g_precomp.bishoppremask);
